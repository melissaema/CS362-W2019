...
// Add refactored code for cards here in their own functions
...
int playAdventurer(struct gameState *state) {
	...
	while (drawntreasure <= 2) { // INTRODUCED BUG: CHANGED < TO <=
...
int playSmithy(struct gameState *state, int handPos) {
	...
	//+3 Cards
	for (i = 0; i <= 3; i++) // INTRODUCED BUG: CHANGED < TO <=
	{
		drawCard(currentPlayer, state);
	}
...
int playMinion(struct gameState *state, int handPos, int choice1, int choice2) {
	...
	if (choice1)		//+2 coins
	{
		state->coins++; // INTRODUCED BUG: CHANGED state->coins + 2 TO state->coins++
	}
...
int playCouncilRoom(struct gameState *state, int handPos) {
	...
	for (i = 0; i < state->numPlayers - 1; i++) // INTRODUCED BUG: CHANGED NUMPLAYERS TO NUMPLAYERS - 1
	{
		if (i != currentPlayer)
		{
			drawCard(i, state);
